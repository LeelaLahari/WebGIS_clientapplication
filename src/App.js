...
function App() {
  ...
  const [attributeNames, setAttributeName] = useState('');
  ...
  const handleSelectedAttributeNameChange = (event) => {
    setAttributeName(event.target.value);
  }
  const handleSelectedAttributeValueChange = (event) => {
  ...
  useEffect(() => {
    fetch(`http://localhost:8080/geoserver/Interop_project/wfs?service=wfs&version=1.1.0&request=DescribeFeatureType&typeName=${selectedFeatures}`)
      .then(response => response.text())
      .then(data => {
  ...
  const vectorSource = new VectorSource({
    ...
    url: function (extent) {
      return (
        'http://localhost:8080/geoserver/Interop_project/wfs?service=WFS&' +
        'version=1.1.0&request=GetFeature&typename=' +
        selectedFeatures +
        '&' +
        'outputFormat=application/json&' +
        'CQL_FILTER=' +
        encodeURIComponent(attributeNames + "='" + attributevalues + "'")
      );
    },
            ...
            <div id="wfs_form" align="left" style={{ width: "45%", margin: "0.5%", padding: "0.5%", marginLeft: "5%" }}>
              <form className="mb-3 row">
                ...
                <div className="mb-3">
                  ...
                  <div className="col-md-6">
                  <label htmlFor="attribute_name" id="attribute_name_label" className="form-label">Filter ID</label>
                  <input id="attribute_name" type="text" className="form-control" value={attributeNames} onChange={handleSelectedAttributeNameChange}></input>
                </div>
                <div className="col-md-6">
                  <label htmlFor="attribute_value" id="attribute_value_label" className="form-label">Filter value</label>
                  <input id="attribute_value" type="text" className="form-control" value={attributevalues} onChange={handleSelectedAttributeValueChange}></input>
                </div>
                </div>
              </form>
